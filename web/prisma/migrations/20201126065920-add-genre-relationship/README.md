# Migration `20201126065920-add-genre-relationship`

This migration has been generated by Yu Taek at 11/26/2020, 3:59:20 PM.
You can check out the [state of the schema](./schema.prisma) after the migration.

## Database Steps

```sql
ALTER TABLE `Tracks_Genres` ADD COLUMN     `trackId` INT,
    ADD COLUMN     `genreId` INT

DROP TABLE `SequelizeMeta`

CREATE INDEX `Tracks_Genres_trackId_foreign_idx` ON `Tracks_Genres`(`trackId`)

CREATE INDEX `Tracks_Genres_genreId_foreign_idx` ON `Tracks_Genres`(`genreId`)

ALTER TABLE `Tracks_Genres` ADD FOREIGN KEY (`trackId`) REFERENCES `Tracks`(`id`) ON DELETE SET NULL ON UPDATE CASCADE

ALTER TABLE `Tracks_Genres` ADD FOREIGN KEY (`genreId`) REFERENCES `Genres`(`id`) ON DELETE SET NULL ON UPDATE CASCADE

ALTER TABLE `Artists_Genres` RENAME INDEX `Artists_Genres_albumId_foreign_idx` TO `Artists_Genres_artistId_foreign_idx`
```
